...
b_changeid()
{
  ...
  outs("購買此權限者，請同意遵守以下規則：\n\n"
       "   1.欲更改ID請於使用者名單按下 ^D\n"
       "   2.更改後的ID 最後方會出現','字元 以示區別 例如 \"chensc,\"\n"
       "   3.由於更改ID後 使用者資料不會顯示 請勿以此戲弄他人\n"
       "   4.請勿將ID改為不雅字眼\n"
       "   5.如違反上述規則 站務得以強迫離線處之 經申訴情節重大者取消權限\n\n"
       "最後還是希望大家能夠玩的愉快^^");

  vget(11, 0, "讀完以上規則，您要花 6 金幣購買更改ID的權限嗎？y/n [n]：", ans, 3, DOECHO);
  if (ans[0] == 'y')
	{																      
	  if (cuser.gold < 6)
	  {
	    vmsg("要 6 金幣才能購買更改ID的權限喔");
	  }
	  else
	  {
	    cuser.gold -= 6;

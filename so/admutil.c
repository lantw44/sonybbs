...
a_resetsys()		/* 重置 */
{
  switch (vans("◎ 系統重設 1)動態看板 2)分類群組 3)指名及擋信 4)全部 S)ync：[Q] "))
  {
  case '1':
    system("bin/camera");
    break;
  ...
  case '4':
	...
	case 's':
		/*
		move(1,0), clrtobot();
		outs("這項功\能會要求作業系統將保留在記憶體中的檔案立即寫入檔案\n"
		     "如果此時有大量資料寫入，可能需要較長的時間\n");
		vmsg(NULL);
		*/
		vmsg("這項功\能會要求作業系統將保留在記憶體中的檔案立即寫入檔案");
		if(fork()==0){
			sync();
			exit(0);
		}else{
  ...
  int fd;
{
  ...
  while (read(fd, &rform, sizeof(RFORM)) == sizeof(RFORM))
  {
    ...
    case 'n':
      ...
      move(9, 0);
      prints("請提出退回申請表原因，或從以下清單選取一項，未輸入直接按 <enter> 則會取消\n\n");

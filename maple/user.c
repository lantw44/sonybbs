...
u_register()
{
  ...
  vmsg("注意：註冊單必須用中文填寫，除非住址、學校或服務單位在國外");
  vmsg("服務單位應避免使用三個字內簡稱，否則有可能被退件");
  memset(&rform, 0, sizeof(RFORM));
  for (;;)
  {
    move (5, 0);
    prints ("\n\033[1;36m《服務單位》\033[m\n");
    prints ("\033[1;30m範例: \033[37m台南一中98級\033[30m " \
           "或 \033[37m台灣\大學資工系97級\033[m，請填「級數」，不要填年級，謝謝。\n");

    getfield(8, 50, rform.career, "服務單位", "學校系級或單位職稱");

    prints ("\n\033[1;36m《目前住址》\033[m\n");
    prints ("\033[1;30m範例: \033[37m台南市東區民族路一段1號\033[30m " \
           "或 \033[37m成功\大學敬一舍1001房\033[m\n");
    getfield(13, 60, rform.address, "目前住址", "包括寢室或門牌號碼");

    prints ("\n\033[1;36m《聯絡電話》\033[m\n");
    prints ("\033[1;30m範例: \033[37m06-2371206\033[30m " \
           "或 \033[37m0912345678\033[m\n");
    getfield(18, 20, rform.phone, "連絡電話", "包括長途撥號區域碼");
...
do_diff(oldfile, filesize, newpath)
	const void* oldfile; /* 儲存就檔案的記憶體位置 */
	const int filesize; /* 舊檔案大小 */
	const char* newpath; /* 新檔案名稱 */
{
	...
	FILE* fp = fdopen(fd, "w");
	sprintf(title,"系統檔案 %s 修改紀錄 (%s)",newpath,cuser.userid);
	fprintf(fp, "作者: %s (%s)\n標題: %s\n時間: %s\n\n",
	...
	if(child == 0){
		close(fdin[1]);
		dup2(fdin[0],0); /* 重新導向 stdin */
		dup2(fd,1); /* 重新導向 stdout */
...
#if 0
	/* lantw44: 不需要等 */
	waitpid(child,NULL,0); /* 等程式跑完 */
#endif
	close(fd);

	/* 這段是從 util/account.c -> keeplog() 抄來的 */
  ...
  char *flist[];		/* filename list */
{
  ...
  n = vget(b_lines, 36, "(D)刪除 (E)編輯 (V)檢視 [Q]取消？", buf, 3, LCECHO);
  if (n != 'd' && n != 'e' && n !='v')
    return;
  ...
  else
  {
	...
	if(mode == M_XFILES){
		if(stat(fpath,&st)<0){
			vmsg("檔案相關資訊讀取失敗，略過執行 diff");
			nodiff=1;
		}else{
			filesize=st.st_size;
			oldfile=malloc(filesize);
			if(oldfile==NULL){
				vmsg("malloc 失敗，略過執行 diff");
				nodiff=1;
			}else{
				FILE* fp;
				if((fp=fopen(fpath,"r"))==NULL){
					vmsg("檔案開啟失敗，略過執行 diff");
					nodiff=1;
				}else{
	...
	noedit=vedit(buf, 0);
    vmsg(noedit ? "原封不動" : "更新完畢");	/* Thor.981020: 注意被talk的問題  */

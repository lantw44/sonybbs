  ...
  FILE *fp;
{
  ...
  {   
    ...
    {
     if (vans("要匿名發表文章嗎(Y/r)？[Y] ") != 'r')   /* 預設改為匿名發表 */
     {
       do
       {
         if (!vget(b_lines, 0, "請輸入你想用的ID，直接按[Enter]用預設、輸入r用真名：",
...
"\033[m                     \033[1m◢▆◣  ▉▃◤ ◢▆◣ ◢▆◣  ◢▆◣\033[m\n" \
"\033[44m        \033[1;33m台南一中\033[;37;44m     █◤  ▕█◥▎ █◤   ◥▆◣  █◤       \033[1;33m索尼小站\033[;37;44m        \033[m\n"\
"                     \033[1m◥▆◤ █ ▕◣ ◥▆◤ ◥▄◤  ◥▆◤\033[m\n" \
"\033[0m   Or\033[1mig\033[30min\033[37m ☆ \033[32msony.tfcis.org\033[m。\033[1;32mbbs.tfcis.org\033[m" \
...
#  ifdef HAVE_REFUSEMARK
  ...
  char *menu2[] = {"SE", "Save     存檔","Bottom   置底存檔", "Abort    放棄", "Title    改標題", "Edit     繼續編輯", "Read     讀取暫存檔", "Write    寫入暫存檔", "Delete   刪除暫存檔", NULL};
  char *menu3[] = {"SE", "Save     存檔","Bottom   置底存檔", "Local    存為站內檔", "XRefuse  加密存檔", "Abort    放棄", "Title    改標題", "Edit     繼續編輯", "Read     讀取暫存檔", "Write    寫入暫存檔", "Delete   刪除暫存檔", NULL};
  char *menu4[] = {"LE", "Local    存為站內檔", "Save     存檔","Bottom   置底存檔", "XRefuse  加密存檔", "Abort    放棄", "Title    改標題", "Edit     繼續編輯", "Read     讀取暫存檔", "Write    寫入暫存檔", "Delete   刪除暫存檔", NULL};
#  else
  char *menu1[] = {"EE", "Abort    放棄", "Title    改標題", "Edit     繼續編輯", "Read     讀取暫存檔", "Write    寫入暫存檔", "Delete   刪除暫存檔", NULL};
...
#  ifdef HAVE_REFUSEMARK
  ...
  char *msg2 = "[S]存檔 (B) 置底 (A)放棄 (T)改標題 (E)繼續 (R/W/D)讀寫刪暫存檔？";
  char *msg3 = "[S]存檔 (B) 置底 (L)站內 (X)密封 (A)放棄 (T)改標題 (E)繼續 (R/W/D)讀寫刪暫存檔？";
  char *msg4 = "[L]站內 (S)存檔 (B) 置底 (X)密封 (A)放棄 (T)改標題 (E)繼續 (R/W/D)讀寫刪暫存檔？";
#  else
  char *msg1 = "[E]繼續 (A)放棄 (R/W/D)讀寫刪暫存檔？";
...
#  endif
  ...
  if (!ans)
  {
    ...
    {
      char xdbuffer[20];
      sprintf (xdbuffer, "請勿使用注音文 (%d)", phoneticsnum);
      //vmsg("請勿使用注音文");
      vmsg (xdbuffer);
      return VE_FOOTER;
    }
...
ve_key:
	...
	break;
      ...
      case Ctrl('Q'):
	cc = vans("顯示使用者資料(1)id (2)暱稱 (3)真實姓名？");
	if (cc >= '1' && cc <= '3')
	{
	  ve_char(KEY_ESC);
	  ve_char('*');
...
#ifdef HAVE_MULTI_BYTE
			case Ctrl('G'): 
	/* lantw44: 全型字偵測切換 (Ctrl 鍵 都被用光了所以用這個) */
	cuser.ufo ^= UFO_ZHC;
  zhc = (cuser.ufo & UFO_ZHC);
	if(zhc){
		vmsg("全型字偵測開啟");

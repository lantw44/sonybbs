...
//  strcpy(cutmp->mateid, userid);
  ...
  prints("\033[1;33m[帳號]\033[m %-12s \033[1;33m[暱稱]\033[m %-16.16s \033[1;33m[上站]\033[m %5d 次 \033[1;33m[文章]\033[m %5d 篇\n",
    userid, acct->username, acct->numlogins, acct->numposts);

  prints("\033[1;33m[認證]\033[m %s通過認證 \033[1;33m[動態]\033[m %-16.16s \033[1;33m[財產]\033[m %s \033[1;33m[個板]\033[m %s\n",
    acct->userlevel & PERM_VALID ? "已經" : "尚未",
    (up && can_see(cutmp, up)) ? bmode(up, 1) : "不在站上",
    fortune[rich],
    (pbrd && !strcmp(pbrd->class, "個人") && pbrd->readlevel != PERM_SYSOP ? "有" : "沒有或隱板")/*,
    (m_query(userid) & STATUS_BIFF) ? "有新信件" : "都看過了"*/ );

  prints("\033[1;33m[來源]\033[m (%s) %s\n",
    Btime(&acct->lastlogin), acct->lasthost);

/*	prints ("\033[1;33m[上站時數]\033[m %s\n", 

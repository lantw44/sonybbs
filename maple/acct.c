	...
	BRD *old, *new;
{
	...
	if(strcmp(old->BM,new->BM)){
		sprintf(buf,"%-13s%s → %s",new->brdname,(strlen(old->BM)) ? old->BM : "(無)",
		                                         (strlen(new->BM)) ? new->BM : "(無)");
		alog("修改板主",buf);
	}
  ...
  int adm;			/* 0: user info  1: admin  2: reg-form */
{
  ...
  prints("  \033[32m出生日期：\033[37m民國 %03d 年 %02d 月 %02d 日            \033[32m我的性別：\033[37m%.2s\n", u->year, u->month, u->day, "？♂♀" + (u->sex << 1));
  ...
  if (adm)
  {
    ...
    bitmsg("  \033[32m習慣旗標：\033[37m", STR_UFO, u->ufo);
		...
		{
		  prints ("  \033[32m註冊住址：\033[37m%s\n", address);
      prints ("  \033[32m服務單位：\033[37m%-35s" \
                "\033[32m註冊電話：\033[37m%s\n",
								career, phone);
		}
  }
  else
  ...
  int adm;
{
  ...
  if (adm)
  {
    adm = vans("設定 1)資料 2)權限 3)檢視註冊單 Q)取消 [Q] ");
    if (adm == '2')
      goto set_perm;
  ...
  int row;
{
  ...
  /* 目前的作法是一輸入完 id 就加入板主權限，即使最後選擇不變動，
	...
	vget(++i, 0, "板主設定 R)一般設定方式 F)自由修改模式？[R] ", buf, 3, LCECHO);

	if(buf[0]=='f'){
		++i;
		outs("\033[1;33m警告：使用此模式修改版主名單會導致受影響的使用者板主權限不正確\033[m\n"
				 "\033[1;33m建議只在板主名單空白時使用，否則修改後請自行處理權限問題\033[m");
		i += 2;
		vget(i, 0, "板主名單：", brd->BM , BMLEN + 1, GCARRY);
  ...
  switch (vget(++i, 0, "看板權限 A)一般 B)自定 C)秘密 D)好友 E)站務隱版？[Q] ", buf, 3, LCECHO))
  {
  case 'c':
  ...
  switch (vget(++i, 0, "看板權限 A)一般 B)自定 E)站務隱板？[Q] ", buf, 3, LCECHO))
  {
#endif
  ...
  int bno;
{
  ...
  switch (vget(8, 0, "(D)刪除 (E)設定 (L)鎖定 (Q)取消？[Q] ", src, 3, LCECHO))
  {
  case 'd':

...
/* 尋找看板函式; 偷自 daemon/bmtad.c -> brd_get () */
/* 只用於 talk.c -> do_query () */
BRD *
  ...
  usint ulevel;
{
  ...
  │        │一般用戶│看板好友│看板壞人│
  ├────┼────┼────┼────┤
  │一般看板│權限決定│  完整  │  水桶  │    看不見：在看板列表中無法看到這個板，也進不去
  ...
  */
  ...
  if (!readlevel || (readlevel & ulevel))
  {
    ...
    if (!postlevel || (postlevel & ulevel))
			/* 看板沒有設定that誰可以發文 || 看板有設定that誰可以發文而且當前用者符合此權限  */
      bits |= BRD_W_BIT;
  }
  else
  {
  ...
  int bno;
{
  ...
  if (currbno != bno)	/* 看板沒換通常是因為 every_Z() 回原看板 */
  {
    ...
    if(stat(fpath, &trash) >= 0){
      /* lock_type = '0' 表示大家都進不去
       * lock_type = '1' 表示鎖定者可進入 */
      ...
      if(lock_perm && !is_admin){
        if(can_enter){
          if(vans("請問您要 [E]進入看板 (M)修改鎖定狀態 ")=='m'){
            brd_editlock(fpath);
          }
        }else{
          if(vans("請問您要修改鎖定狀態嗎？[N]")=='y'){
            brd_editlock(fpath);
          }
          return -1;
        }
...
void brd_editlock(const char* fpath){
  ...
  switch(vans("L)鎖定 M)修改理由 U)解鎖？[Q]")){
    case 'l':
        if(stat(fpath,&trash)>=0){
          vmsg("這個看板目前已經處於鎖定狀態");
          break;
        }
        if(vans("鎖定類型 (1)允許\自己可以進入看板 [2]所有人皆無法進入看板？")=='1'){
          type = '1';
        }else{
          type = '0';
        }
        vget(b_lines, 0, "理由：", reason, 48, DOECHO);
    ...
    case 'm':
        ...
        if(HAS_PERM(PERM_ALLBOARD)){
          vget(b_lines, 0, "由誰鎖定：", userid, IDLEN + 1, GCARRY);
        }
        vget(b_lines, 0, "理由：", reason, 48, GCARRY);
        if(vans(msg_sure_ny)=='y'){
          brd_setlockinfo(fpath,type,userid,reason);
        }
      break;
...
void brd_viewlock(const char* fpath){
 /* move(19,0),clrtobot(),refresh(); */
  ...
  char buf[80];
  sprintf(buf, "鎖定者：%s", userid);
  vmsg(buf);
  sprintf(buf, "理由：%s", reason);
  vmsg(buf);
}
/*
  prints("\033[1;33m[由誰鎖定]\033[m %s\n", userid);
  prints("\033[1;33m[鎖定理由]\033[m %s\n", reason);
*/
...
//    str3 = "\033[1;31m爆\033[m";
  if (bno > 30)
    str3 = "\033[1;33;41m爆\033[m";
  else if (bno >= 20)
    str3 = "\033[1;33m發\033[m";
  else if (bno >= 10)
    str3 = "\033[1;31m發\033[m";
	else if (bno >= 5)

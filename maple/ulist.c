  ...
  int sysop;
{
  ...
  {
    strcpy(buf, "\033[1;31m  獸猩\033[m");
  }
  else
  {
    /* duck.050729: 老人標記.. */
		if (up->userlevel & PERM_ROBOT)
			strcpy (buf, "\033[1;34m  機器\033[m");
		else
    if (up->userlevel & PERM_ELDER)
      strcpy (buf, "\033[32m  老人\033[m");
    else 
    if (up->userlevel & PERM_ALLADMIN)
			strcpy (buf, "\033[1;33m  工友\033[m");
		else
  ...
  XO *xo;
{
  ...
  if(strcmp(userid,realid)){
    if(HAS_PERM(PERM_ALLADMIN) || cuser.userno == userno){
	  ...
	  vmsg("這位使用者使用了改 ID 權限，但您沒有權限檢視原始 ID");
	}
  }else{
    my_query(userid);
  }
  ...
  XO *xo;
{
  ...
  {
    char ans[20];
    if (vget(b_lines, 0, "要給他幾個水球？", ans, 10, DOECHO))
    {

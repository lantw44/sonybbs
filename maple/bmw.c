...
#define BMW_FORMAT3 "\033[1;33;45m★%s \033[34;47m %s \033[m" /* 收到的廣播 */
#define BMW_FORMAT4 "\033[1;33;44m★%s \033[34;47m %s \033[m" /* Robot 水球 */
    ...
    do
    {
     ...
     // fprintf(fp, BMW_FORMAT " %s\n", mptr->userid, mptr->msg, Btime(&mptr->btime));
      ...
      bmw_lslot[locus++] = *mptr;	/* structure copy */
			...
			/* 當 NPC 收到別人丟水球後會自動回話 */
			if (!strcmp(cuser.userid, "Robot"))  /* Robot 的 ID 是 Robot */
			{
				if (mptr->caller)
				{
					BMW new;
